...
fn completed() -> PaymentDrainSummary {
    PaymentDrainSummary::new(true, 0, 2, 1)
}
...
async fn create_and_lookup_return_the_same_closed_redacted_aggregate() {
    ...
    let expected = r#"{"status":"completed","accepted_count":0,"terminal_count":2,"cancellation_enqueued_count":1}"#;
...
async fn drain_failures_use_stable_coarse_error_envelopes() {
    ...
    for (error, status, expected) in [
        ...
        .oneshot(signed_request(&key, "/payment-request-drains", body()))
        .await
        .unwrap();
